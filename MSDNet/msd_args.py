...
model_names = ['msdnet', 'msdnet_ge', 'IMTA_MSDNet', 'mobilenet_imagenet','ranet','resnet18_cifar']
...
arch_group.add_argument('-nB','--nBlocks', type=int, default=5)
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
arch_group.add_argument('--nChannels', type=int, default=32)
arch_group.add_argument('--base', type=int, default=4)
arch_group.add_argument('--stepmode', type=str, default='even', choices=['even', 'lin_grow','lg'])
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
...
mood_group.add_argument('-pv', '--pvalue', type=int, 
                    default=1,
                    help='method of classifying')

mood_group.add_argument('-multi', '--multitest', type=str, 
                    default='BH',
                    help='method of multi-test:BH,adaBH,BY,Fisher,Cauchy,all')

mood_group.add_argument('-temp', '--temperature', type=float, 
                    default=1,
                    help='temperature of energy score')
...
# RANet模型增加参数--------------------------------------------------------
mood_args.scale_list = list(map(int, mood_args.scale_list.split('-')))
#-------------------------------------------------------------------------
