...
from utils.MOOD import auroc,fpr95,get_curve_multi,fpr_auc_multi
...
if 1:#load and test model
    from msd_args import mood_args
    import models
    ...
    model.load_state_dict(torch.load(mood_args.file)['state_dict'],strict=False) ##已修改
...
i_score, i_adjusted_score, i_complexity = get_ood_score(data_name=mood_args.id,
                           ...
                           mean=mean,
                           cal_complexity=True,
                           T=mood_args.temperature     #修改超参数T
                           )
auroc_base = []
fpr95_base = []
auroc_mood = []
...
for o_name in mood_args.od:
    ...
    if mood_args.pvalue:
        ...
        I_pvalue_l = []
        #=====================
        for j in range(mood_args.layer):
            
            rank = ss.rankdata(np.insert(i_score[j],0,o_score[j]))
            Pvalue = (rank+1)/(len(i_score[j])+len(o_score[j])+1)
            O_pvalue = Pvalue[:len(o_score[j])]
            I_pvalue = Pvalue[len(o_score[j]):]
            #=========储存===================
            O_pvalue_l.append(O_pvalue)
            I_pvalue_l.append(I_pvalue)
            #================================
            auroc_l.append(auroc(I_pvalue, O_pvalue))
            fpr95_l.append(fpr95(I_pvalue, O_pvalue))
        auroc_base.append(auroc_l)
        fpr95_base.append(fpr95_l)
        auroc_for_barplot.append(auroc_l)
#=============================================================================================
        rank = ss.rankdata(np.insert(i_adjusted_score,0,o_adjusted_score))
        Padjvalue = (rank+1)/(len(o_adjusted_score)+len(i_adjusted_score)+1)
        O_adjpvalue = Padjvalue[:len(o_adjusted_score)]
        I_adjpvalue = Padjvalue[len(o_adjusted_score):]
        
        try:
            auroc_mood.append(auroc(I_adjpvalue, O_adjpvalue))
        except ValueError:
            auroc_mood.append(0)
    
        fpr95_mood.append(fpr95(I_adjpvalue, O_adjpvalue))
        
        #多重检验方法
        auroc_multi,fpr95_multi = get_curve_multi(O_pvalue_l,I_pvalue_l,
                                num_layer=mood_args.layer,method=mood_args.multitest)
        auroc_Multi.append(auroc_multi)
        fpr95_Multi.append(fpr95_multi)
    else:
        for j in range(mood_args.layer):
            auroc_l.append(auroc(i_score[j], o_score[j]))
            fpr95_l.append(fpr95(i_score[j], o_score[j]))
        auroc_base.append(auroc_l)
        fpr95_base.append(fpr95_l)
...
print('********** ',mood_args.arch,':auroc result ',mood_args.id,' with ',mood_args.score,' **********')


if mood_args.multitest != 'all':
        
    print('                              auroc                              fpr95    ')
    print('OOD dataset      exit@last    MOOD        {}         exit@last    MOOD      {}'.format(mood_args.multitest,
                                                                                                 mood_args.multitest))
    for i in range(len(mood_args.od)):
        data_name=mood_args.od[i]
        data_name = data_name + ' '*(17-len(data_name))
        print(data_name,"%.4f"%auroc_base[i][-1],'   ',"%.4f"%auroc_mood[i],'    ',"%.4f"%auroc_Multi[i],'    ',"%.4f"%fpr95_base[i][-1],'   ',"%.4f"%fpr95_mood[i],'   ',"%.4f"%fpr95_Multi[i])
    data_name = 'average'
    data_name = data_name + ' '*(17-len(data_name))
    print(data_name,"%.4f"%np.mean(list(list(zip(*map(reversed,auroc_base)))[0])),'   ',
          "%.4f"%np.mean(auroc_mood),'    ',"%.4f"%np.mean(auroc_Multi),'    ',
          "%.4f"%np.mean(list(list(zip(*map(reversed,fpr95_base)))[0])),'   ',
          "%.4f"%np.mean(fpr95_mood),'   ',"%.4f"%np.mean(fpr95_Multi))
else:
    
    print('                                       ****auroc****                              ')
    print('OOD dataset       exit@last      MOOD      BH        adaBH      BY       Fisher      Cauchy')
    for i in range(len(mood_args.od)):
        data_name=mood_args.od[i]
        data_name = data_name + ' '*(17-len(data_name))
        print(data_name,"%.4f"%auroc_base[i][-1],'   ',"%.4f"%auroc_mood[i],'    ',"%.4f"%auroc_Multi[i][0],'    ',"%.4f"%auroc_Multi[i][1],'   ',"%.4f"%auroc_Multi[i][2],'   ',"%.4f"%auroc_Multi[i][3],'   ',"%.4f"%auroc_Multi[i][4])
    data_name = 'average'
    data_name = data_name + ' '*(17-len(data_name))
    print(data_name,"%.4f"%np.mean(list(list(zip(*map(reversed,auroc_base)))[0])),'   ',
          "%.4f"%np.mean(auroc_mood),'    ',"%.4f"%np.mean([i[0] for i in auroc_Multi]),'    ',
    ...
    for i in range(len(mood_args.od)):
        data_name=mood_args.od[i]
        data_name = data_name + ' '*(17-len(data_name))
        print(data_name,"%.4f"%fpr95_base[i][-1],'   ',"%.4f"%fpr95_mood[i],'    ',"%.4f"%fpr95_Multi[i][0],'    ',"%.4f"%fpr95_Multi[i][1],'   ',"%.4f"%fpr95_Multi[i][2],'   ',"%.4f"%fpr95_Multi[i][3],'   ',"%.4f"%fpr95_Multi[i][4])
    data_name = 'average'
    data_name = data_name + ' '*(17-len(data_name))
    print(data_name,"%.4f"%np.mean(list(list(zip(*map(reversed,fpr95_base)))[0])),'   ',
          "%.4f"%np.mean(fpr95_mood),'    ',"%.4f"%np.mean([i[0] for i in fpr95_Multi]),'    ',
...
print("=====================single exit=====================================")
print('OOD dataset           auroc          fpr95')
for i in range(len(mood_args.od)):
    for j in range(mood_args.layer):
        data_name=mood_args.od[i]+"-exit@"+str(j)
        data_name = data_name + ' '*(20-len(data_name))
        print(data_name,"%.4f"%auroc_base[i][j],'   '*3,"%.4f"%fpr95_base[i][j])
    print("*******************************************")
...
if mood_args.score == 'energy' and mood_args.adjusted == 1 :
    ...
    Flops = np.zeros([8])#选择8个OOD数据集##################################################################################
    for i in range(8):
        Flops[i] = np.sum(selected_score[i,:]*flops)
    Flops2 = np.ones([8])*flops[-1]
