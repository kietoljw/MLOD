...
#         out = self.layer4[-1].forward_threshold(out, threshold=1)
        out = self.layer4(out)
        out_list.append(out)
        out = self.avgpool(x)
...
class ResNetCifar(AbstractResNet):
    def __init__(self, block, layers, num_classes=100, method='', p=None, info=None):
...
def resnet152_cifar(**kwargs):
    return ResNetCifar(Bottleneck, [3, 8, 36, 3], **kwargs)

def resnet18_cifar(**kwargs):
    return ResNetCifar(BasicBlock, [2,2,2,2], **kwargs)


def resnet34_imagenet(pretrained,**kwargs):
...
def resnet50_imagenet(pretrained,**kwargs):
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    return model

def resnet101_imagenet(pretrained,**kwargs):
...
def resnet18_imagenet(pretrained,**kwargs):
    model = ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model