...
def cal_p_value(known, novel):

    results = dict()
    AUROC, fpr_at_tpr95, in_p, out_p = get_curve_pvalue(known, novel)
    mtypes = ['FPR', 'AUROC']
...
def print_results(results, datasets, method):
    mtypes = ['TPR', 'FPR', 'AUROC']
    ...
    for result, dataset in zip(results, datasets):
        ...
        print(' {val:6.2f}'.format(val=100. * result['TPR']), end='')
...
def print_all_results(results, datasets, method):
    mtypes = ['FPR', 'AUROC'] #mtypes = ['FPR', 'AUROC', 'AUIN']#['FPR', 'DTERR', 'AUROC', 'AUIN', 'AUOUT']
...
def compute_average_results(all_results):
    mtypes = ['FPR', 'AUROC'] #mtypes = ['FPR', 'DTERR', 'AUROC', 'AUIN', 'AUOUT']
    avg_results = dict()
...
def get_curve_pvalue(known, novel, threshold=0.05):
    ...
    return auroc, fpr_at_tpr95, known, novel
